...
// Only static assets here to avoid runtime URL bugs.
const HERO_MEDIA = [
  '/assets/wedding(5).jpg.jpeg',
  '/assets/wedding(6).jpg.jpeg',
  '/assets/wedding(7).jpg.jpeg',
  '/assets/wedding(8).jpg.jpeg',
  '/assets/wedding (9).jpg.jpeg',
  '/assets/Graduation.jpg.jpeg',
  '/assets/Graduation.jpg (3).jpeg',
  '/assets/wedding.jpg.jpeg',
  '/assets/studio (4).jpg.jpeg',
  '/assets/studio (18).jpg.jpeg',
  '/assets/outdoor.jpg.jpeg',
  '/assets/Outdoor.jpg (2).jpeg',
  '/assets/pregnancy.jpg.jpeg',
  '/assets/pregnancy.jpg (2).jpeg',
  '/assets/studio (10).jpg.jpeg',
  '/assets/studio (20).jpg.jpeg',
  '/assets/family.jpg.jpeg',
  '/assets/Nadia video 1.mp4',
  '/assets/pre wedding.mp4',
];
...
export default function Home() {
  ...
  // Dynamic hero visible
  useEffect(() => {
    const timer = setTimeout(() => setHeroVisible(true), 100);
    return () => clearTimeout(timer);
  }, []);

  // Keep your existing fetch logic (services/testimonials/portfolio)
  useEffect(() => {
  ...
  // NOTE: portfolio items from API may include image_url/file_path; keep resilient.
  const getImgSrc = (item) => {
    const src = item?.image || item?.image_url || item?.thumbnail_path;
    if (!src) return '/assets/studio.jpg.jpeg';
  ...
  return (
    <div className="bg-[#0a0a0a] text-white min-h-screen font-inter">
      <style>
        {`
          ...
          .animate-infinity-scroll {
            animation: infinityScroll 400s linear infinite;
          }
        `}
      </style>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
        {/* Sliding background (infinite-like loop) */}
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden bg-black">
          <div className="flex h-full w-max animate-[infinityScroll_40s_linear_infinite]">
            {[...Array(2)].flatMap((_, repeatIndex) =>
              HERO_MEDIA.map((src, index) => (
                <div
                  key={`${repeatIndex}-${index}`}
                  className="w-[100vw] sm:w-[500px] md:w-[600px] lg:w-[800px] flex-shrink-0 h-full border-r border-black/40"
                >
                  {src.endsWith('.mp4') ? (
                    <video src={src} autoPlay loop muted playsInline className="w-full h-full object-cover" />
                  ) : (
                    <img src={src} alt="Gallery Background" className="w-full h-full object-cover" />
                  )}
                </div>
              ))
            )}
        ...
        >
          <div className="inline-flex items-center gap-2 bg-orange-500/10 border border-orange-500/30 rounded-full px-5 py-2 mb-8">
            ...
            <span className="text-orange-400 text-sm font-medium tracking-widest uppercase">Kigali&apos;s Premier Photography Studio</span>
          </div>
          ...
          >
            ...
            <span className="relative inline-block">
              <span className="bg-gradient-to-r from-orange-400 via-orange-500 to-amber-400 bg-clip-text text-transparent">Moments</span>
            </span>
            <br />
            That Last a{' '}
            <span className="bg-gradient-to-r from-orange-400 to-amber-300 bg-clip-text text-transparent">Lifetime</span>
          </h1>

          <p className="text-gray-400 text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed">
            Professional photography services tailored to preserve your most precious memories. Award-winning photographers serving Kigali and all of Rwanda.
          </p>

          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
            <Link
              to={
                isAuthenticated ? (user?.role === 'admin' ? '/admin/dashboard' : '/services') : '/register'
              }
        ...
        <div className="absolute bottom-0 left-0 right-0 z-10 border-t border-white/5 bg-black/40 backdrop-blur-md">
          <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4">
            {STATS.map((stat, i) => (
              <div key={i} className="flex flex-col items-center justify-center py-5 px-4 border-r border-white/5 last:border-r-0">
                <span className="text-2xl md:text-3xl font-bold text-orange-400" style={{ fontFamily: "'Playfair Display', serif" }}>
                  {stat.value}
                </span>
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-white mt-3 mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>
            Our Services
          </h2>
          <p className="text-gray-400 max-w-xl mx-auto">From intimate studio sessions to grand wedding ceremonies, we cover every milestone with artistry.</p>
        </div>
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-orange-400 font-semibold text-sm">From {service.price || `RWF ${service.starting_price?.toLocaleString?.()}`}</span>
                  <Link
      ...
      <section className="py-24 px-6 bg-black/30">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-gray-400 max-w-xl mx-auto">A glimpse into the stories we&apos;ve had the privilege of telling through our lens.</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {portfolio.slice(0, 8).map((item, index) => (
              <div
                key={item.id}
                className={`group relative overflow-hidden rounded-2xl cursor-pointer ${index === 0 ? 'sm:row-span-2' : ''}`}
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-6 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                  <span className="text-orange-400 text-xs font-semibold tracking-widest uppercase mb-1">{item.category}</span>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.slice(0, 3).map((t) => (
              <div
                key={t.id}
                className="bg-white/3 border border-white/8 rounded-2xl p-8 hover:border-orange-500/40 transition-all duration-500"
              >
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-12 h-12 rounded-full bg-orange-500/15 border border-orange-500/30 flex items-center justify-center text-orange-400 font-bold">
                    {t.avatar}
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-1 mb-4">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <svg
                      key={i}
                      className={`w-4 h-4 ${i < (t.rating || 0) ? 'text-orange-400' : 'text-white/20'}`}
                      viewBox="0 0 24 24"
                      fill="currentColor"
      ...
      <section className="py-20 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-r from-orange-500/10 to-transparent p-10 md:p-14">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(249,115,22,0.35),transparent_45%)]" />
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
              <div>
                <div className="text-orange-400 text-sm font-semibold tracking-[0.3em] uppercase">Ready to book?</div>
                <h2 className="font-playfair text-4xl md:text-5xl font-bold mt-3" style={{ fontFamily: "'Playfair Display', serif" }}>
                  Let’s create your next unforgettable story.
                </h2>
                <p className="text-gray-400 mt-4 max-w-xl">
                  Tell us what you’re planning and we’ll help you choose the perfect session package.
                </p>
              </div>

              <div className="flex flex-col sm:flex-row gap-4">
                <Link
                  to={isAuthenticated ? '/my-bookings' : '/register'}
                  className="bg-orange-500 hover:bg-orange-400 text-black font-bold px-8 py-4 rounded-full text-lg transition-all duration-300"
                >
                  {isAuthenticated ? 'View My Bookings' : 'Create Account'}
                </Link>
                <Link
                  to="/contact"
                  className="bg-white/5 border border-white/10 hover:border-orange-500/40 hover:bg-orange-500/10 text-white font-semibold px-8 py-4 rounded-full text-lg transition-all duration-300"
                >
                  Contact Us
                </Link>
              </div>
